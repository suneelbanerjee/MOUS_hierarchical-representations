...
outdir = '/home/neel/Documents/SPM_results/alternate_parameter_contrasts/SPM-V_Lg10_mean_bigram_plus1'
if ~exist(outdir,'dir')
    mkdir(outdir)
end
cd(subject_path) 
...
for v=1:length(subjNames)
    ...
    % matlabbatch{1}.spm.spatial.smooth.prefix = 'G';
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).name = 'Mean Bigram Frequency';
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).param = 0 - (regressors.log10_Mean_plus1_Bigram - mean(regressors.log10_Mean_plus1_Bigram));
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).poly = 1;
    ...
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = char(strcat("Mean Bigram Frequency Correlation"));
