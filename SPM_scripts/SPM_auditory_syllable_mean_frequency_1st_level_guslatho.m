...
outdir = '/home/neel/Documents/SPM_results/alternate_parameter_contrasts/SPM-A_syllables_guslatho_mean';
mkdir(outdir)
...
for m = 1:length(subjNames)
    ...
    regressors = readtable(char(fullfile(sourcedir, currentName, 'func', strcat('guslatho_syllabified.csv'))),'Delimiter',',');
    disp(strcat("Number of onsets  = ", num2str(height(regressors))));

    % Log transform the specified column
    regressors.Mean_Freq_Count = regressors.Mean_Log10_FreqCount;
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).param = 0 - (regressors.Mean_Freq_Count - mean(regressors.Mean_Log10_FreqCount))
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).poly = 1;
    ...
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = 'Mean Syllable Frequency';
