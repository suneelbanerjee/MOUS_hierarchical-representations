...
%Same logic as first_syllable_frequency_1st_level_guslatho.m, but using the RESTRICTED set of word onsets
%(only words whose first syllable is NOT the minimum-frequency syllable, i.e. First_syllable_zipf ~= MinSyllZipf),
%read from {subject}_first_syllable_frequency_restricted.csv.
subject_path = '/mnt/MOUSnew/fmriprep_fresh';
outdir = '/mnt/MOUSnew/SPM_results/FIRST/first_syllable_restricted';
mkdir(outdir)
...
for m = 1:length(subjNames)
    ...
    regressors = readtable(char(fullfile(sourcedir, currentName, 'func', strcat(currentName,'_first_syllable_frequency_restricted.csv'))),'Delimiter',',');
    ...
    %regressor 2, first syllable frequency (demeaned and sign-flipped)
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).name = 'First Syllable Frequency';
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).param = 0 - (regressors.First_syllable_zipf - mean(regressors.First_syllable_zipf))
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).poly = 1;
