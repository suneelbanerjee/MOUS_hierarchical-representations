%1st Level Visual analysis. Same logic as first_bigram_1st_level.m, but using the RESTRICTED set of
%word onsets (only words whose first bigram is NOT the minimum-frequency bigram,
%i.e. First_bigram_Zipf ~= Min_bigram_Zipf), read from {subject}_first_and_min_bigram_restricted.csv.
%Single parametric modulator: first bigram frequency (demeaned, sign-flipped), orth = 0.
subject_path = '/mnt/MOUSnew/fmriprep_fresh';
source = '/mnt/MOUSnew/SynologyDrive/source/SynologyDrive'
outdir = '/mnt/MOUSnew/SPM_results/FIRST/first_bigram_restricted'
...
for v=1:length(subjNames)
    ...
    regressors = readtable(char(fullfile(source, currentName, 'func', strcat(currentName,'_first_and_min_bigram_restricted.csv'))),'Delimiter',',');
    %1. File Selection
    try
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).name = 'First Bigram Frequency';
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(1).param = 0-(regressors.First_bigram_Zipf - mean(regressors.First_bigram_Zipf))
    ...
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = char(strcat("First Bigram Frequency Correlation"));
