...
outdir = '/home/neel/Documents/SPM_results/SPM-A_syllables_IPA_guslatho_log10_lengthcontrol';
mkdir(outdir)
...
for m = 1:length(subjNames)
    ...
    regressors = readtable(char(fullfile(sourcedir, currentName, 'func', 'guslatho_syllabified.csv')),'Delimiter',',');
    disp(strcat("Number of onsets  = ", num2str(height(regressors))));

    % Bring in the right column
    regressors.Min_Freq_Count = regressors.MinLog10FreqCount;
    %disp(regressors)
    ...
    lexical_table = readtable(char(fullfile(sourcedir,currentName,'func',strcat(currentName, '_transcription.csv'))))
    lexical_table{inf_rows,numericVars} = 0;
    durations = lexical_table.Duration
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond.pmod(1).param = durations - mean(durations) %demean
