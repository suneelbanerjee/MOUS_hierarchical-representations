...
outdir = '/mnt/MOUSnew/SPM_results/DUAL_CONTRAST_length_controlled/visual_word_len1_plus1minBG1_WF1'
mkdir(outdir)
cd(subject_path) 
subjects = dir('sub-V*');
...
for v=1:length(subjNames)
    ...
    if exist(fullfile(subject_path,currentName,'func'))
        ...
        %REGRESSOR 2: BIGRAM FREQUENCY (MIN). Do confirm the r2 between minBGfreq and Zipf before usng it as a regressor here- dont want to destroy the entire effect!
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(2).name = 'Min BG Frequency';
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(2).param = 0 - (sublex_regressors.log10_Min_plus1_Bigram - mean(sublex_regressors.log10_Min_plus1_Bigram));
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(3).param = 0-(word_regressors.Zipf - mean(word_regressors.Zipf));
