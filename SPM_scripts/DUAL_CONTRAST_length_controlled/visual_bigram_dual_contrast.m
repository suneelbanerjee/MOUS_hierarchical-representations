...
outdir = '/mnt/MOUSnew/SPM_results/DUAL_CONTRAST_length_controlled/visual_bigram_len1_minBG1_WF1'
...
for v=1:length(subjNames)
    ...
    if exist(fullfile(subject_path,currentName,'func'))
        try
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(2).param = 0- (word_regressors.Zipf - mean(word_regressors.Zipf));
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(2).poly = 1;

            %REGRESSOR 3: BIGRAM FREQUENCY (MIN). %Do confirm the r2 between minBGfreq and Zipf before usng it as a regressor here- dont want to destroy the entire effect!
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(3).name = 'Min BG Frequency';
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod(3).param = 0-(sublex_regressors.log10_Min_Bigram -mean(sublex_regressors.log10_Min_Bigram));
            ...
            matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = char(strcat("Bigram Frequency Correlation (controlling word length and word frequency)"));
            ...
            disp(strcat("Subject ", currentName, " not found." ))
        end
    end
end
