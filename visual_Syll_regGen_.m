home = '/home/neel/Desktop/phoneme_freq_regressor';
cd(home)
...
sourcepath = '/media/neel/MOUS/MOUS/MOUS/SynologyDrive/source';
cd(sourcepath)
subjects = dir('sub-A*');

for i = 1:length(subjects)
    subject = char(subjects(i).name);
    eventsTSV = fullfile(sourcepath,subject,'func',strcat(subject, '_task-visual_events.tsv'));

    %pick out the audio files
    events = tdfread(eventsTSV);
    for j = 1:length(string(events.type))
        if strtrim(string(events.type(j,:))) == "Picture"
            display = events.value(j,:);
            if isstrprop(display(:,1),'digit') 
    ...
    relevant_events.dmFirst = num2cell(first_double - mean(first_double));
    ...
    tablename = strcat(subject, "_phon_freqRegresors.xlsx");
    cd(subject)
    cd('func')
    writetable(relevant_events,tablename)
    cd(sourcepath)
    disp(strcat(subject, " regressors saved"))
end
