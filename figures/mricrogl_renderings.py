...
for subdir in os.listdir(master_dir):
    ...
    if os.path.isdir(subdir_path):  # Ensure it's a directory
        ...
        nii_path = os.path.join(subdir_path, 'spmT_0001.nii')  # Keep three 0s in filename
        ...
        print(f"Checking {nii_path}: {'Exists' if os.path.exists(nii_path) else 'MISSING'}")
        ...
        if os.path.exists(nii_path) and os.path.exists(u_threshold_path) and os.path.exists(cluster_centroids_path):
            ...
            gl.overlayload(nii_path)  # Load the spmT_0001.nii file
            ...
            left_view_path = os.path.join(subdir_path, 'left_view.png')
            gl.savebmp(left_view_path)
            print(f"Saved Left view: {left_view_path}")

            gl.azimuthelevation(270, 0)  # Right view
            right_view_path = os.path.join(subdir_path, 'right_view.png')
            gl.savebmp(right_view_path)
            print(f"Saved Right view: {right_view_path}")
