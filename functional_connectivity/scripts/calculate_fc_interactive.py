...
import numpy as np
import pandas as pd
import readline  # Optional: Adds command history and better input handling on Linux/Mac
...
# Base path for subject data (adjust if needed)
...
def setup_analysis_params():
    ...
    # 1. Select Task
    while True:
        task_choice = input("1. Select Task (1=Auditory, 2=Visual): ").strip()
        if task_choice == "1" or task_choice.lower() == "auditory":
            config["name"] = "auditory"
            config["subj_glob"] = "sub-A*"
            config["task_label"] = "task-auditory"
            print("   >> Selected: AUDITORY task.")
            break
        elif task_choice == "2" or task_choice.lower() == "visual":
            config["name"] = "visual"
    ...
    while True:
        ...
        if roi_path.exists() and roi_path.is_dir():
            ...
            if niftis:
                ...
                print(f"   [!] Directory contains no .nii/.nii.gz files.")
        else:
...
if __name__ == "__main__":
    ...
    # Structure: OUTPUT_ROOT / TASK / ROI_SET_NAME
    # e.g., ./results/auditory/lexical_clusters
    ...
    # 4. Find Subjects
    # Note: BASE_DIR is still hardcoded at the top of the script. 
    # If you want that interactive too, let me know!
    ...
    # We will save results into ./results/auditory/ROI_DIR_NAME to allow organizing different ROI sets
    roi_folder_name = params["roi_dir"].name
    task_output_dir = OUTPUT_ROOT / params["name"] / roi_folder_name
    
    print(f"\n[Info] Output Directory: {task_output_dir}")
    task_output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for subj_path in subjects:
        ...
        if bold_file.exists():
            ...
            print(f"Missing BOLD for {subject_label} (Checked: {bold_file.name}) — skipped.")
